...
const orderCtrl = {
    ...
    try {
      ...
      for (let item = 0; item < orderItems.length; item++) {
        const productItem = await Products.findOne({
          _id: orderItems[item].product_id,
        });
        if (productItem) {
            ...
            if (type_id == product.types[i]._id) {
              if (amount > product.types[i].amount) {
                ...
                for (var j = 0; j < order.listOrderItems.length; j++) {
                  if (
                    order.listOrderItems[j].type_id == type_id &&
                    order.listOrderItems[j].product_id == product_id
                  ) {
                    //amount
                    if (
                      order.listOrderItems[j].amount + amount >
                      product.types[i].amount
                    ) {
    ...
    try {
      ...
      for (let item = 0; item < orderItems.length; item++) {
        ...
        if (productItem) {
          ...
          const itemPrice = await typeCtrl.getPricebyId(
            orderItems[item].type_id
          );
          if (itemPrice === 0) {
            res.send({ msg: "Wrong type id" });
            return;
          }
          const itemAmount = await typeCtrl.getAmountbyId(
            orderItems[item].type_id
          );
          if (itemAmount === 0) {
  ...
  checkoutOrder: async (req, res) => {
    ...
    if (!order) {
      ...
      const amount = (order.total / 25400.8).toFixed(2);
  ...
  updateOrderDetail: async (req, res) => {
    ...
    if (!phone || !name || !address || !order_id) {
      res
        .status(400)
        .json({ message: "phone, name, address and order_id are required" });
      return;
    }
    try {
      const userId = await authMe(req);
  ...
  removeOrderItem: async (req, res) => {
    ...
    try {
      ...
      if (!order) {
        ...
        const index = listOrderItems.findIndex(
  ...
  updateDelive: async (req, res) => {
    ...
    if (!delivery_id || !order_id) {
      res
        .status(400)
        .json({ message: "delivery_id and order_id are required" });
      return;
    }
    try {
      await Orders.findOneAndUpdate(
